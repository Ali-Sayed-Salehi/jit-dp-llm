...
threshold_str = threshold_time.strftime("%Y-%m-%dT%H:%M:%SZ")

os.makedirs(os.path.dirname(all_bugs_path), exist_ok=True)

# 1) fetch and write all_bugs.csv row by row
with open(all_bugs_path, "w", newline="", encoding="utf-8") as f_all:
    writer_all = csv.DictWriter(f_all, fieldnames=FIELDS)
...
# 2) read back from disk (your original pattern)
...
all_bugs_list = all_bugs_df_sorted.to_dict(orient="records")
...
for bug in regressions_list:
    ...
    for bug_id in regressor_bug_ids_list:
        regressor_bug = {
            ...
            "alert_date": bug["alert_creation_date"],
        }
...
for bug in all_regressor_bugs_list:
    # Skip old bugs components and products
    created_date = bug.get("alert_date")
    created_date_ts = pd.to_datetime(created_date, utc=True)
    if created_date_ts < start_cutoff:
        continue
...
# 3) write perf_bugs.csv row by row
os.makedirs(os.path.dirname(perf_bugs_path), exist_ok=True)
perf_fields = [
    "bug_id",
    "bug_summary",
    "bug_is_perf_regressor",
    "bug_is_perf_regression",
    "regressed_perf_tests",
    "regressor_revision",
    "perf_reg_alert_summary_id",
    "product",
    "component",
    "creation_time",
]

with open(perf_bugs_path, "w", newline="", encoding="utf-8") as f_perf:
    writer_perf = csv.DictWriter(f_perf, fieldnames=perf_fields)
    ...
    for bug in all_bugs_list:
        ...
        # skip bugs older than START_DATE_ISO
        created_ts = pd.to_datetime(bug_creation_time, utc=True)
        if created_ts < start_cutoff:
            continue
        ...
        writer_perf.writerow(bug_with_added_info)
