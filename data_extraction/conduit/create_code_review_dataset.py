...
With `--write-select-commit-diffs`, it also writes one JSON object per selected
commit to:

    datasets/mozilla_code_review/select_commit_diffs.jsonl

Each output row contains:

    commit_id, dataset_split, risk_score, bug_inducing, drev_submission_date,
    drev_closed_date, drev_author, files_changed, reviews

Each optional select-commit-diffs row contains:

    commit_id, diff, commit_message
...
DEFAULT_RISK_SCORES_JSONL = DATASET_DIR / "per_commit_risk_scores.jsonl"
DEFAULT_OUTPUT_JSONL = DATASET_DIR / "drev_review_data.jsonl"
DEFAULT_SELECT_COMMIT_DIFFS_JSONL = DATASET_DIR / "select_commit_diffs.jsonl"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Also write a JSONL file with commit_id, raw Mercurial diff, and "
            "raw Mercurial commit message for each selected commit."
        ),
    )
    parser.add_argument(
        "--select-commit-diffs-jsonl",
        default=str(DEFAULT_SELECT_COMMIT_DIFFS_JSONL),
        help="Path to write select_commit_diffs.jsonl when requested.",
    )
    parser.add_argument(
        "--autoland-repo",
...
def parse_binary_label(value: Any, *, line_label: str, field: str) -> int:
    ...
    if isinstance(value, float) and value.is_integer() and int(value) in (0, 1):
        return int(value)
    raise ValueError(f"{line_label}: invalid {field} {value!r}")
...
def load_risk_annotations(path: Path) -> dict[str, CommitRiskAnnotation]:
    ...
    for item in iter_jsonl(path):
        ...
        if commit_id in annotations:
            raise ValueError(f"{line_label}: duplicate commit_id {commit_id!r}")

        if "risk_score" not in record:
            raise ValueError(f"{line_label}: missing risk_score")
        if "bug_inducing" not in record:
            raise ValueError(
                f"{line_label}: missing bug_inducing. Regenerate "
                "per_commit_risk_scores.jsonl with get_commit_risk_scores.py."
            )
...
def validate_rows_have_risk_annotations(
    rows: list[InputRow],
    risk_annotations_by_commit: dict[str, CommitRiskAnnotation],
) -> None:
    missing_commit_ids = sorted(
        row.commit_id
        for row in rows
        if row.commit_id not in risk_annotations_by_commit
    )
    if missing_commit_ids:
        preview = ", ".join(missing_commit_ids[:5])
        raise ValueError(
            "per_commit_risk_scores.jsonl is missing "
            f"{len(missing_commit_ids)} DREV transaction commit ids. "
...
def load_required_commit_metadata(
    path: Path,
    ...
    commit_metadata: dict[str, CommitMetadata] = {}
    commit_index = 0
    for item in iter_jsonl(path):
        ...
        if node in required_commit_ids:
            if node in commit_metadata:
                raise ValueError(f"{path}:{item.line_num}: duplicate commit node {node}")

            desc = item.record.get("desc", "")
            if not isinstance(desc, str):
                raise ValueError(
                    f"{path}:{item.line_num}: commit {node} has invalid desc"
                )

            commit_metadata[node] = CommitMetadata(
                commit_index=commit_index,
                commit_message=desc,
            )
            if len(commit_metadata) == len(required_commit_ids):
                break

        commit_index += 1

    missing_commit_ids = sorted(required_commit_ids - set(commit_metadata))
    if missing_commit_ids:
        preview = ", ".join(missing_commit_ids[:5])
        raise ValueError(
...
def get_commit_diff(repo_path: Path, commit_id: str) -> str:
    return run_hg_capture(repo_path, ["diff", "--change", commit_id])
...
def build_output_row(
    *,
    ...
    annotation = risk_annotations_by_commit[commit_id]
    ...
    with output_jsonl.open("w", encoding="utf-8") as output_file:
        for input_row in rows:
            output_row = build_output_row(
                ...
                risk_annotations_by_commit=risk_annotations_by_commit,
...
def write_select_commit_diffs(
    *,
    rows: list[InputRow],
    commit_metadata_by_id: dict[str, CommitMetadata],
    autoland_repo: Path,
    output_jsonl: Path,
) -> dict[str, int]:
    stats = {
        "rows_seen": len(rows),
        "rows_written": 0,
        "unique_commits_read": 0,
    }
    commit_diffs_by_id: dict[str, str] = {}

    output_jsonl.parent.mkdir(parents=True, exist_ok=True)
    with output_jsonl.open("w", encoding="utf-8") as output_file:
        for input_row in rows:
            commit_id = input_row.commit_id
            commit_message = commit_metadata_by_id[commit_id].commit_message
            diff = commit_diffs_by_id.get(commit_id)
            if diff is None:
                diff = get_commit_diff(autoland_repo, commit_id)
                commit_diffs_by_id[commit_id] = diff
                stats["unique_commits_read"] += 1
...
def main(argv: list[str] | None = None) -> None:
    ...
    select_commit_diffs_jsonl = Path(args.select_commit_diffs_jsonl)
    ...
    commit_metadata_by_id = load_required_commit_metadata(
        all_commits_jsonl,
        {row.commit_id for row in rows},
    )
    commit_indices_by_id = {
        commit_id: metadata.commit_index
        for commit_id, metadata in commit_metadata_by_id.items()
    }
    risk_annotations_by_commit = load_risk_annotations(risk_scores_jsonl)
    validate_rows_have_risk_annotations(rows, risk_annotations_by_commit)
    ...
    if args.write_select_commit_diffs:
        select_commit_diff_stats = write_select_commit_diffs(
            rows=rows,
            commit_metadata_by_id=commit_metadata_by_id,
            autoland_repo=autoland_repo,
            output_jsonl=select_commit_diffs_jsonl,
        )
        stats.update(
            {
                f"select_commit_diffs_{key}": value
                for key, value in select_commit_diff_stats.items()
            }
        )
