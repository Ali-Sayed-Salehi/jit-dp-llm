...
- Outside the window the simulator sets risk to “unavailable” and strategies are not allowed to probe earlier than `window_start`.
...
#### NLB: No lookback
Do **no additional tests** to find a good boundary; use the earliest available commit:

$$
g = \text{window\_start}
$$

This forces bisection to search over the widest possible interval within the risk window.
