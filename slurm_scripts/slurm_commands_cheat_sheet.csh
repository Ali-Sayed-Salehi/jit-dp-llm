...
# CCDB
/scratch/repos/perf-pilot
module load cuda/12.6; module load python/3.13.2; source /scratch/repos/perf-pilot/scripts/set_env.sh;
...
# VIRYA
source /etc/profile.d/modules.sh; module load python/3.13.2; module load cuda/12.6.3; module list;

# 20 GB MIGS - Virya[3-6]
salloc --partition=migs --gpus=1 --mem=100G --time=01:00:00

# 32 GB GPU (8) - Virya[1-2]
salloc --partition=phys --gpus=1 --mem=100G --time=01:00:00
...
cd /speed-scratch/$USER/vscode ; setenv XDG_RUNTIME_DIR /speed-scratch/$USER/run-user ; module load python/3.12.0/default ; module load cuda/11.8/default ; source /speed-scratch/$USER/repos/perf-pilot/scripts/set_env.csh; /speed-scratch/nag-public/code-server-4.22.1/bin/code-server --user-data-dir=$PWD\/projects --config=$PWD\/home/.config/code-server/config.yaml --bind-addr="0.0.0.0:8890" /speed-scratch/$USER/repos/perf-pilot


# submit a batch job
sbatch /speed-scratch/$USER/repos/perf-pilot/slurm_scripts/speed/seq_class_llama.sh
...
sbatch /speed-scratch/a_s87063/repos/perf-pilot/slurm_scripts/speed/download_transform_model.sh
